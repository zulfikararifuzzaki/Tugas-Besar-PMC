...
int getTransient(ekivalen *rangkaianx){
	FILE *outputI;
	FILE *outputVR;
	FILE *outputVC;
	outputI = fopen("transienI.txt", "w");
	outputVR = fopen("transienVR.txt", "w");
	outputVC = fopen("transienVC.txt", "w");
	

	float time = 0;
	float dt = 0.1;
	float tau = rangkaianx->R * rangkaianx->C;
	float euler = 2.7183;
	float tempI, tempVR, tempVC;
	float Io = rangkaianx->V / rangkaianx->R;
	
	printf("R= %f, C = %f\n", rangkaianx->R, rangkaianx->C);
	printf("Tau = %f\n", tau);
	while(time < 5*tau){
		time += dt;
		tempI = Io * pow(euler, -1*time/tau);
		tempVR = rangkaianx->R * tempI;
		tempVC = rangkaianx->V - tempVR;
		printf("%f;%f;%f\n", time, tempI, tempVR);
		fprintf(outputI, "%f;%f\n", time, tempI);
		fprintf(outputVR, "%f;%f\n", time, tempVR);
		fprintf(outputVC,"%f;%f\n", time, tempVC);
		}
